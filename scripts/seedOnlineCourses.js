...
const getIconName = (title) => {
    if (title.includes('DICCIÓN'))
        return 'ChatBubbleBottomCenterTextIcon';
    if (title.includes('LOCUCIÓN'))
        return 'MegaphoneIcon';
    if (title.includes('ORATORIA'))
        return 'PresentationChartLineIcon';
    if (title.includes('NARRACIÓN'))
        return 'ChatBubbleBottomCenterTextIcon'; // Usamos uno similar
    if (title.includes('ACENTO NEUTRO'))
        return 'MicrophoneIcon';
    if (title.includes('DOBLAJE'))
        return 'SpeakerWaveIcon';
    if (title.includes('AUDIOLIBROS'))
        return 'SpeakerWaveIcon'; // Usamos uno similar
    return 'MicrophoneIcon'; // Default
};
// Función para parsear la fecha
const parseCourseDate = (dateString) => {
    const monthMap = {
...
const seedCourses = async () => {
    ...
    try {
        ...
        const jsonPath = path.resolve(__dirname, '../public/cursos-proximos-online');
        const jsonData = fs.readFileSync(jsonPath, 'utf-8');
        const data = JSON.parse(jsonData);
        const coursesToSeed = data.courses;
